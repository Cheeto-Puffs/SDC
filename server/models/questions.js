...
module.exports = {
  ...
  addQuestion: (product_id, question) => {
    ...
    return pool.query(`INSERT INTO questions (product_id, q_body, q_date_written, asker_name, asker_email, q_reported, q_helpful) VALUES (${product_id}, '${question.body}', ${Date.now()}, '${question.asker_name}', '${question.asker_email}', 'f', 0)`)
    .then(res => {
      console.log(res)
    })
  ...
  markQuestionHelpful: (question_id) => {
    return pool.query(`UPDATE questions SET q_helpful = q_helpful + 1 WHERE q_id = ${question_id}`)
    .then(res => console.log(res))
    .catch(err => console.log('error updating helpful for question in models: ', err))
  },
  reportQuestion: (question_id) => {
    return pool.query(`UPDATE questions SET q_reported = 't' WHERE q_id = ${question_id}`)
    .then(res => console.log(res))
